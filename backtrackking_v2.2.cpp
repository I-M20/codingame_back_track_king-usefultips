...
// - Infinite 'WAIT' turns bug resolved
...
public:
    ...
    // How far past the deadline the search actually ran, in microseconds.
    // The outOfTime() breaks stop the sweeps, but the work already in flight
    // when one fires still has to finish, so this is what the budget checks
    // actually cost in the worst case.
    long long overrunUs;
...
public:
    ...
    void run(ActionSet &outAction, int &outDisrupt)
    {
        ...
        // How far the search ran past its own deadline. The breaks above stop
        // the sweeps, but the work already in flight when one fires still has
        // to finish, so this is what the checks actually cost in the worst
        // case. Zero when the search finished inside the budget.
        {
